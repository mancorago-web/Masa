...
function FamiliarPicker({ onAdd, onBack }: { onAdd: (name: string, price: number) => void; onBack: () => void }) {
  const [selected, setSelected] = useState<string[]>([]);

  const allPizzas = defaultRecipes
    .filter(r => ['PIZZAS CLÁSICAS', 'PIZZAS VEGETARIANAS', 'PIZZAS ESPECIALES'].includes(r.category))
    .map(r => r.name);
...
export default function Ventas() {
  ...
  const [showMix2x1, setShowMix2x1] = useState(false);
  const [mixPizza1, setMixPizza1] = useState('');
  const [mixPizza2, setMixPizza2] = useState('');
  const [showFamiliarPicker, setShowFamiliarPicker] = useState(false);
            ...
            <div className="bg-white rounded-lg w-full max-w-2xl shadow-xl max-h-[85vh] flex flex-col">
              ...
              {modalTab === 'menu' ? (
                showHalfPicker ? (
                  ...
                  <FamiliarPicker
                    onAdd={(name, price) => addItem(name, price)}
                    onBack={() => setShowFamiliarPicker(false)}
                  />
                ) : (
                          ...
                          <div className="divide-y divide-gray-100">
                            {cat.type === 'pizza' ? cat.pizzas.map(pizza => (
                              ...
                              <div key={group.name}>
                                ...
                                {group.name === 'Pizzas 2x1 (8 Pzas.)' && (
                                  <button
                                    onClick={() => {
                                      const classicPizzas = productCategories
                                        .filter(c => c.type === 'pizza')
                                        .flatMap(c => c.pizzas)
